...
contract("Rubicon Market and Pools Original Tests", async function (accounts) {
  let newPair;
  ...
  describe("Bath House Initialization of Bath Pair and Bath Tokens", async function () {
    ...
    it("Bath Token for asset is deployed and initialized", async function () {
      ...
      assert.equal(await bathAssetInstance.RubiconMarketAddress(), rubiconMarketInstance.address);
    });
    it("Bath Token for quote is deployed and initialized", async function () {
      ...
      assert.equal(await bathQuoteInstance.RubiconMarketAddress(), rubiconMarketInstance.address);
    });
  ...
  describe("Liquidity Providing Tests", async function () {
    ...
    it("scrubStrategistTrades can be used by approved strategists", async function () {
      ...
      assert.equal(
        (
          await bathPairInstance.getOutstandingStrategistTrades(WETHInstance.address, DAIInstance.address, accounts[0])
        ).length.toString(),
        "3"
      );
    });
    it("bathTokens are correctly logging outstandingAmount", async function () {
      ...
      const outCount = await bathPairInstance.getOutstandingStrategistTrades(WETHInstance.address, DAIInstance.address,accounts[0]);
