...
contract("Rubicon Router Test", async function (accounts) {
  ...
  describe("Deployment", async function () {
    ...
    it("Router is deployed with correct exchange address", async function () {
      ...
      await router.startErUp(
        (
          await RubiconMarket.deployed()
        ).address,
  ...
  describe("WETH Function Tests", async function () {
    // To properly test, only need an ASSET/WETH market that ganache accounts can interact with using local ETH
    let rubiconMarketInstance = await RubiconMarket.deployed();
  ...
  describe("Random", async function () {
    ...
    it("WIP - getBook returns data on the real-time order book", async function () {
      ...
      let asks = await result[0];
      for (let index = 0; index < asks.length; index++) {
        const pay = await asks[0][index];
        const buy = await asks[1][index];
        // logIndented("this is an ask?", element.toNumber());
      }
      // logIndented("final result", result);
    });
  });
});
